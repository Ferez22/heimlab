...
export default function IndexPage() {
  ...
  const scrollYPadded = useTransform(scrollY, (v) =>
    sat(
      v /
        (captionWrapperSizeRef.current.clientRect.height -
          windowSizeRef.current.windowHeight)
    )
  );

  const scrollYPaddedQuicker = useTransform(scrollYPadded, (v) =>
    remap(0, quickerPoint, 0, 1, v)
  );
  const scrollYPaddedRest = useTransform(scrollYPadded, (v) =>
    remap(quickerPoint * 2, quickerPoint * 3, 0, 1, v)
  );
  const scrollYPaddedClose = useTransform(scrollYPadded, (v) =>
    remap(quickerPoint * 4, 1, 0, 1, v)
  );
